...
//		}
		...
		메로리에서 소멸된다. 여기서 사용된 변수 j는 지역변수(local variable)
		...
		안쪽(좁은지역)에서 사용할 수 있다. 즉 아래에 선언한 변수i는
		main 메소드의 지역변수가 된다.
		 */
		int i=0;
		for( ; i<=5 ; i++) {
		...
		for(int x=2 ; x<=9 ; x++) {
			for(int y=1 ; y<=9 ; y++) {
				System.out.printf("%-2d*%-2d=%-2d%n",x, y, x*y);
			}
		}
